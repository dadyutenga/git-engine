...
func (NodeStrategy) Deploy(project domain.Project, exec application.RemoteExecutor) error {
	cmd := fmt.Sprintf("cd %s && npm install --production && (pm2 restart %s || pm2 start npm --name %s -- start)", project.DeployDir, project.Name, project.Name)
	_, err := exec.Run(cmd)
	return err
}
...
func (NodeStrategy) Restart(project domain.Project, exec application.RemoteExecutor) error {
	_, err := exec.Run(fmt.Sprintf("pm2 restart %s", project.Name))
	return err
}
...
func (NodeStrategy) Status(project domain.Project, exec application.RemoteExecutor) (bool, error) {
	out, err := exec.Run(fmt.Sprintf("pm2 describe %s", project.Name))
	if err != nil {
		return false, err
	}
